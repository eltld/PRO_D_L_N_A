...
/**
 * Wraps a <tt>Device</tt> for display with icon and label. Equality is implemented with UDN comparison.
 * 
 */
public class DeviceItem {
    ...
    public Drawable getIcon() {
        if (device.getIcons( ).length > 0) {
            MyLog.d( "DeviceItem", "  icon=" + device.getIcons( )[0].toString( ) );
            // device.getIcons( )[0].
        }
        return icon;
    }
    ...
    public String getIconStr() {
        if (device.getIcons( ).length > 0) {
            return device.getIcons( )[0].getUri( ).toString( );
        }
        return null;
    }
