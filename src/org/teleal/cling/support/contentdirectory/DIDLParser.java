 ...
 * </p>
 * <p>
 * Override the {@link #createDescMetaHandler(org.teleal.cling.support.model.DescMeta, org.teleal.common.xml.SAXParser.Handler)} method to read vendor extension content of
 * {@code <desc>} elements. You then should also override the {@link #populateDescMetadata(org.w3c.dom.Element, org.teleal.cling.support.model.DescMeta)} method for writing.
 * </p>
 * <p>
 * Override the {@link #createItemHandler(org.teleal.cling.support.model.item.Item, org.teleal.common.xml.SAXParser.Handler)} etc. methods to register custom handlers for
 * vendor-specific elements and attributes within items, containers, and so on.
 * </p>
 * 
...
public class DIDLParser extends SAXParser {
     ...
     * <p>
     * Optionally, items inside a container will be represented in the XML, the container elements then have nested item elements. Although this parser can read such a structure,
     * it is unclear whether other DIDL parsers should and actually do support this XML.
     * </p>
     * 
     * @param content
     *            The content model.
    ...
    protected void generateRoot(DIDLContent content, Document descriptor, boolean nestedItems) {
        ...
        rootElement.setAttributeNS( "http://www.w3.org/2000/xmlns/", "xmlns:didl", DIDLObject.Property.DLNA.NAMESPACE.URI );
    ...
    protected void generateResource(Res resource, Document descriptor, Element parent) {
        ...
        Element resourceElement = appendNewElement( descriptor, parent, "res", MediaServer.LocalURL + resource.getValue( ) );
     ...
     * {@link #createDescMetaHandler(org.teleal.cling.support.model.DescMeta, org.teleal.common.xml.SAXParser.Handler)}, you most likely also want to override this method.
     * </p>
     * 
     * @param descElement
    ...
    protected void appendProperties(Document descriptor, Element parent, DIDLObject object, String prefix, Class<? extends DIDLObject.Property.NAMESPACE> namespace,
            String namespaceURI) {
...
//            log.warning( "DIDLObjectHandler  uri=" + uri + "\tlocalName=" + localName + "\tqName+" + qName + "\t attributes=" + attributes );
//            printAtts( attributes );
            if (DIDLObject.Property.DC.NAMESPACE.URI.equals( uri )) {
                ...
                if ("writeStatus".equals( localName )) {
                    ...
                    DIDLObject.Property albumArtURI = null;
                    try {
                        albumArtURI = new DIDLObject.Property.UPNP.ALBUM_ART_URI( URI.create( getCharacters( ) ) );
                    ...
                    for (int i = 0; i < albumArtURIAttributes.getLength( ); i++) {
                        if ("profileID".equals( albumArtURIAttributes.getLocalName( i ) ) && albumArtURI != null) {
                            albumArtURI.addAttribute( new DIDLObject.Property.DLNA.PROFILE_ID( new DIDLAttribute( DIDLObject.Property.DLNA.NAMESPACE.URI, "dlna",
...
//            log.warning( "RootHandler  uri=" + uri + "\tlocalName=" + localName + "\tqName+" + qName + "\t attributes=" + attributes );
//            printAtts( attributes );
    ...
    public class ContainerHandler extends DIDLObjectHandler<Container> {
        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
            super.startElement( uri, localName, qName, attributes );
//            log.warning( "ContainerHandler  uri=" + uri + "\tlocalName=" + localName + "\tqName+" + qName + "\t attributes=" + attributes );
...
//            printAtts( attributes );
            ...
            if (DIDLObject.Property.UPNP.NAMESPACE.URI.equals( uri )) {
                ...
                if ("searchClass".equals( localName )) {
                    getInstance( ).getSearchClasses( ).add(
                            new DIDLObject.Class( getCharacters( ), getAttributes( ).getValue( "name" ), "true".equals( getAttributes( ).getValue( "includeDerived" ) ) ) );
    ...
    public class ItemHandler extends DIDLObjectHandler<Item> {
        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
            super.startElement( uri, localName, qName, attributes );
//            log.warning( "ItemHandler  uri=" + uri + "\tlocalName=" + localName + "\tqName+" + qName + "\t attributes=" + attributes );
//            printAtts( attributes );
            if (!DIDLContent.NAMESPACE_URI.equals( uri ))
                return;
    ...
    public class DescMetaHandler extends Handler<DescMeta> {
        ...
        public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {
            super.startElement( uri, localName, qName, attributes );
//            log.warning( "DescMetaHandler  uri=" + uri + "\tlocalName=" + localName + "\tqName+" + qName + "\t attributes=" + attributes );
//
////            printAtts( attributes );
...
//    public void printAtts(Attributes attributes) {
//        if (attributes == null)
//            return;
//        MyLog.e( TAG, "================================" );
//        for (int i = 0; i < attributes.getLength( ); i++) {
//            MyLog.e( TAG, "QName=" + attributes.getQName( i ) + "    LocalName=" + attributes.getLocalName( i ) + "   value=" + attributes.getValue( i ) );
//        }
//        MyLog.e( TAG, "================================" );
//
//    }
