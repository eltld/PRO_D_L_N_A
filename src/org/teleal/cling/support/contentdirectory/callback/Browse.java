...
public abstract class Browse extends ActionCallback {
    ...
    public Browse(Service service, String containerId, BrowseFlag flag) {
        this( service, containerId, flag, CAPS_WILDCARD, 0, 0 );
    }
     ...
     */
    public Browse(Service service, String objectID, BrowseFlag flag, String filter, int firstResult, int maxResults, SortCriterion... orderBy) {
        ...
        getActionInvocation( ).setInput( "RequestedCount", new UnsignedIntegerFourBytes( maxResults == 0 ? getDefaultMaxResults( ) : maxResults ) );
    ...
    public void success(ActionInvocation invocation) {
        ...
        BrowseResult result = new BrowseResult( invocation.getOutput( "Result" ).getValue( ).toString( ), (UnsignedIntegerFourBytes) invocation.getOutput( "NumberReturned" )
                .getValue( ), (UnsignedIntegerFourBytes) invocation.getOutput( "TotalMatches" ).getValue( ), (UnsignedIntegerFourBytes) invocation.getOutput( "UpdateID" )
                .getValue( ) );
        Log.e( "Browse", "success=" + result.toString( ) );
        ...
        if (proceed && result.getCountLong( ) > 0 && result.getResult( ).length( ) > 0) {
            try {
                ...
                Log.e( "Browse", "Exception   =" + ex.getMessage( ) );
